...
public class PostRepositoryImp implements IPostRepository {
    ...
    public Set<PostDTO> getPostsById(Set<Integer> listPostId) {
        return listPostId.stream().map(p -> mapPosts.get(p)).collect(Collectors.toSet());
    }
    @Override
    ...
    public Map<Integer, PostDTO> loadPost(){
        posts.add(new Post(1, 5, LocalDate.of(2022, 12, 29), 1, 2, 2000,false,0));
        posts.add(new Post(2, 5, LocalDate.of(2022, 12, 28), 2, 2, 2000,false,0));
        posts.add(new Post(3, 7, LocalDate.of(2022, 11, 29), 3, 2, 2000,false,0));
        posts.add(new Post(4, 6, LocalDate.of(2022, 11, 27), 4, 2, 2000,false,0));
        ...
        ).category(2).price(2000).has_promo(true).discount(0.25).build());
