...
import com.mercadolibre.bootcamp_demo_java_app.dtos.CurrencyConvertDTO;
...
public class ItemsApiService extends RestClientService{
    ...
    private String convertApiBaseUrl;
    ...
    public Double getConvertRate(ItemDTO itemInfo) throws ParseException, RestException{
        convertApiBaseUrl = String.format("%s/currency_conversions/search?from=%s&to=%s",baseUrl,itemInfo.getCurrencyId(),"USD");
        Response resp = itemsApiClient.get(convertApiBaseUrl);
        CurrencyConvertDTO currencyConvertDTO = resp.getData(CurrencyConvertDTO.class);
        return currencyConvertDTO.getRate();
    }
}
