...
import com.socialmeli.be_java_hisp_w20_g8.dto.SellerPromotionsDTO;
...
public class PostService implements IPostService {
    ...
    public PostService() {
        ...
        mapper.createTypeMap(PostRequestDTO.class, Post.class)
                .addMapping(src -> src.getProductDTO().getProduct_id(), Post::setProduct_id);
    }
    ...
    public boolean createPost(PostRequestDTO postRequestDTO) {
        ...
        if(!(postRequestDTO.isHas_promo()==true && postRequestDTO.getDiscount()>0)){
            postRequestDTO.setHas_promo(false);
            postRequestDTO.setDiscount(0);
        }
        ...
        PostDTO postDTO = new PostDTO(post.getUser_id(), post.getDate(), productService.getProductById(post.getProduct_id()), post.getCategory(), post.getPrice(),
                post.isHas_promo(), post.getDiscount());
    ...
    @Override
    public SellerPromotionsDTO CountProductsWithDiscount(String user_id) {
        Seller seller = personRepository.findSellerById(Integer.valueOf(user_id));
        if(seller == null)
            throw new NotFoundException("The specified seller does not exist in the database");
        Set<Integer> idsPosts = seller.getPost();
        long countPromoPost = postRepository.getPostsById(idsPosts).stream().filter(post -> post.isHas_promo()).count();
        SellerPromotionsDTO sellerPromotionsDTO = new SellerPromotionsDTO(seller.getId(), seller.getUser_name(),
                countPromoPost);
