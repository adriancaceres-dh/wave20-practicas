...
public class PostService implements IPostService {
    ...
    @Override
    public boolean createPromoPost(PromoPostRequestDTO promoPostRequestDTO) {
        // Check if all the fields are present
        if(!Stream.of(promoPostRequestDTO.getUser_id(),
                        promoPostRequestDTO.getDate(),
                        promoPostRequestDTO.getProductDTO(),
                        promoPostRequestDTO.getCategory(),
                        promoPostRequestDTO.getPrice(),
                        promoPostRequestDTO.isHas_promo(),
                        promoPostRequestDTO.getDiscount())
                .allMatch(Objects::nonNull))
            throw new InvalidArgumentException("All the fields are required");

        // Get the seller
        Seller seller = personRepository.findSellerById(promoPostRequestDTO.getUser_id());
        ...
        // Create the product if it doesn't exist
        productService.createProduct(promoPostRequestDTO.getProductDTO());

        // Create the post
        Post post = mapper.map(promoPostRequestDTO, Post.class);
        // Create the post DTO
        PostDTO postDTO = new PostDTO(post.getUser_id(), post.getDate(), productService.getProductById(post.getProduct_id()), post.getCategory(), post.getPrice());
    ...
    public PromoPostCountDTO promoPostCount(int userId) {
        ...
        Integer numberOfPromoPost = (int) seller.getPost().stream().filter(x-> postRepository.getPostById(x).isHas_promo()).count();
        return PromoPostCountDTO.builder()
                .user_name(seller.getUser_name())
                .user_id(seller.getId())
                .promo_products_count(numberOfPromoPost)
                .build();
    ...
    @Override
    public PromoPostListDTO listPromoPostCount(int user_id) {
        Seller seller = personRepository.getById(user_id);
        if (seller == null) {
            throw new NotFoundException("User not found: " + user_id);
        }
        List<PromoPostRequestDTO> promoPostRequestDTOList = seller.getPost().stream()
                .filter(x-> postRepository.getPostById(x).isHas_promo())
                .map(x-> mapper.map(postRepository.getPostById(x),PromoPostRequestDTO.class))
                .collect(Collectors.toList());
        return PromoPostListDTO.builder()
                .user_name(seller.getUser_name())
                .user_id(seller.getId())
                .posts(promoPostRequestDTOList)
                .build();
    }
}
