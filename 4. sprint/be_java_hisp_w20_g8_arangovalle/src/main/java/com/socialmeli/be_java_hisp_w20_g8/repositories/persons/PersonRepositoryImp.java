...
public class PersonRepositoryImp implements IPersonRepository {
    ...
    @Override
    public boolean addNewUser(UserDTO newUser){

        this.persons.get("users").add(new User(newUser.getId(), newUser.getUser_name(), new HashSet<>()));
        return true;

    }

    @Override
    public Set<Person> allUsers(String userType){
        return this.persons.get(userType);
    }
