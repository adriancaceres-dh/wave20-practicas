...
public class LoadExampleData {
    ...
    private void initializePostsAndProducts() {
        ...
        Product sillaPromo = new Product(1002, "MesaPromo", "Mueble", "Vaca mistica", "Blanco", "");
        Product mesaPromo = new Product(1003, "SillaPromo", "Gamer", "Noblechairs", "Rosa", "");
        ...
        Post tercerPost = new Post(LocalDate.now(), sillaPromo, categoryRepository.findByCode(200).get(), 120000, true, 0.25);
        Post cuartoPost = new Post(LocalDate.now(), mesaPromo, categoryRepository.findByCode(200).get(), 120000, true, 0.1);


        postRepository.save(primerPost);
        postRepository.save(segundoPost);
        postRepository.save(tercerPost);
        postRepository.save(cuartoPost);

        flavio.addPost(primerPost);
        flavio.addPost(segundoPost);
        ale.addPost(tercerPost);
        ale.addPost(cuartoPost);
